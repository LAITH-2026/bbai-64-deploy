...
`depth_m` field to every detection; disable it with `--no-depth`.

> **Why depth isn't a model.** The original plan offloaded Depth-Anything-V2, but
> it is a DINOv2 **ViT + DPT dense head**, and TIDL on this SDK only validates
> *classification* transformers — the dense head + attention/LayerNorm/GELU ops
> fall back to the A72 and crawl (seconds/frame). Since the runtime only ever
> needs **one distance per YOLO box** (not a dense map), depth is now computed in
> closed form from the known CARLA camera geometry: **ground-plane / IPM** for
> road objects and **known-size pinhole** for mounted signs/lights. This runs on
> the A72 in microseconds, is natively metric, and **frees the C7x** (it now runs
> only YOLO + UFLD). See `runtime/depth_runtime.py` and `config.py` (`class DEPTH`).
...
The TDA4VM has **one** C7x+MMA (8 TOPS), **not two**. The two CNN models therefore
**time-share a single accelerator** — inference is serial, so combined C7x latency
= `yolo + ufld` (depth adds nothing to the C7x; it is A72 geometry). The win over
the Windows/CUDA baseline comes from:

1. **INT8 quantization on the MMA** — the dominant speedup.
2. **Pipeline overlap** — A72 cores run preprocessing / NMS / `pred2coords` /
   per-box depth geometry / compositing for one frame while the C7x runs inference
   for the next. The single-GPU `integrate.py` pays all of that serially; here it
   is hidden.
...
> **Depth is now free on the C7x.** It used to be the heavy one — Depth-Anything-V2
> is a ViT/DINOv2 transformer TIDL couldn't offload. It is replaced by closed-form
> monocular geometry on the A72 (microseconds, native metres), so the accelerator
> runs only YOLO + UFLD. `--no-depth` still skips the `depth_m` field if you want
> it gone, but it no longer buys any C7x headroom.
...
| transformers                | ❌ (no depth model) | ❌ **never**    |
| edgeai-tidl-tools           | ✅                  | ❌              |
...
per-object depth geometry are **pure numpy / math** (`runtime/pred2coords_np.py`,
`runtime/yolo_runtime.py`, `runtime/depth_runtime.py`). Depth needs **no** PC step
at all — there is nothing to export or compile for it.
...
- **Depth** — no model to swap. Tune the camera geometry and per-class sizes in
  ...
  holds the real-world height (metres) per detection class for the pinhole
  estimator. `GROUND_CONTACT` lists the classes ranged by the ground-plane (IPM)
  method. Calibrate these against CARLA's ground-truth depth sensor.
...
- The **JSON schema is identical** in all three modes (it is exactly the MQTT
  message): `{frame, timestamp, lanes_detected, lanes:[{index, points}], objects:[{class_id, class_name, confidence, bbox_xyxy, depth_m}]}`. Object boxes and lane points are both in **frame-pixel coordinates**; `depth_m` is the per-object metric distance in **metres** (`null` when depth is disabled, or the box is beyond `MAX_RANGE_M` / edge-truncated and can't be estimated reliably).
