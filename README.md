...
(Depth-Anything-V2 → per-object distance in metres) — for the **BeagleBone AI-64
(TI TDA4VM)**, running on the **C7x+MMA** accelerator via **TIDL** and fed by
**MQTT frames from CARLA**.
...
| Depth-Anything-V2 checkpoint | — | Auto-downloaded from Hugging Face by `export/export_depth_onnx.py` (PC only). |
...
python bbai64-deploy/export/export_depth_onnx.py
...
python bbai64-deploy/compile/compile_depth_tidl.py
